# OPP-054: System contacts + canonical `who` — identity graph, phones, cross-channel agents
...
### 1. Contacts as sources (align with [OPP-051](OPP-051-unified-sources-mail-local-files-future-connectors.md))

- **`googleContacts`** — Google **People API**, OAuth scopes added to existing Google auth ([OPP-042](OPP-042-google-oauth-cli-auth.md)); incremental sync token per account; normalized rows in SQLite.
