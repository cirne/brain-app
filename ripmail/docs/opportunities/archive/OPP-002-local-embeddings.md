...
- Cache model in `RIPMAIL_HOME/data` (or similar) instead of `~/.cache` so it travels with the deployment volume? Or document the HuggingFace cache location so users know where it lives.
