...
```bash
ripmail search "query"         # Always runs semantic + FTS (current "hybrid")
ripmail search "query" --fts   # FTS only, for exact keyword matching
```
...
- Update tool documentation for MCP `search_mail` (in-tree MCP was later removed; see [OPP-039](../OPP-039-mcp-deferred-cli-first.md))
...
**MCP tool docs (historical; `docs/MCP.md` removed):**
...
**Help Text:**
- `ripmail search --help`: Remove "Search modes" section, add `--fts` flag
- `ripmail --help`: Remove mode hint, update search command description
