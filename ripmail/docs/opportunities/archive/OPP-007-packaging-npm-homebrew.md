...
**Package name:** `@cirne/zmail` (scoped). The unscoped name `ripmail` is already taken on npm (old package, ~3 weekly downloads). Packages are published to public npm. Install via `npm i -g @cirne/zmail` (no authentication required).
...
- User runs `npm i -g @cirne/zmail` or `bun install -g @cirne/zmail`; when they run `ripmail`, the OS invokes Bun with that script.
...
- **Distribute via npm** so that `npm i -g @cirne/zmail` (scoped package name; unscoped `ripmail` is taken on npm) is the canonical install. This matches OpenClaw and Claude Code and fits the target user.
...
- **TypeScript:** Keep writing TS. For the published package, either:
  ...
  - Ship TS and use **tsx** as the bin (e.g. `bin: { "ripmail": "dist/run.js" }` where `run.js` is a small wrapper that uses `tsx` or `node --loader ts-node/esm` to run the TS entry). Many CLIs ship JS for simplicity and smaller install.
...
GitHub Actions workflow (`.github/workflows/release-builds.yml` today; historical name `release.yml`) automatically built and published releases:
