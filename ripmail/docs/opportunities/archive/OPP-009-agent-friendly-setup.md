...
**Problem:** The current `ripmail setup` flow requires an interactive TTY and synchronous human interaction via readline prompts. This doesn't work for stdio-based agent interactions (like Claude Code, Cursor, or other AI assistants) where stdin isn't a TTY and the agent needs to provide credentials programmatically.

When an agent tries to set up ripmail, it hits a wall: `ripmail setup` detects non-interactive mode and exits with instructions to manually create config files, which defeats the purpose of agent automation.
...
**Example:** An agent wants to set up ripmail for a user. It has:
- Email address: `user@gmail.com`
- Gmail app password: `abcd efgh ijkl mnop`
- OpenAI API key: `sk-...`

Currently, the agent must manually create `~/.ripmail/config.json` and `~/.ripmail/.env` files, which is error-prone and doesn't leverage the validation logic built into `ripmail setup`.

**Proposed direction:** Split setup into two distinct workflows:
1. **`ripmail setup`** — Non-interactive, agent-friendly setup via CLI flags/env vars (like AWS CLI, GitHub CLI)
2. **`ripmail wizard`** — Interactive, human-friendly TUI wizard (like OpenClaw's `openclaw setup --wizard`)
...
### `ripmail setup` — Non-interactive, agent-friendly

**Design:** When all required values are provided via CLI flags or environment variables, `ripmail setup` runs non-interactively without prompts. This follows patterns from AWS CLI, GitHub CLI, and other agent-friendly tools.

**CLI flags (preferred for explicit agent control):**
```bash
ripmail setup --email user@gmail.com --password "abcd efgh ijkl mnop" --openai-key "sk-..."
```

**Environment variables (fallback for CI/automation):**
```bash
RIPMAIL_EMAIL=user@gmail.com RIPMAIL_IMAP_PASSWORD="..." RIPMAIL_OPENAI_API_KEY="sk-..." ripmail setup
```
...
### `ripmail wizard` — Interactive, human-friendly TUI
...
### Phase 1: Non-interactive `ripmail setup`

1. **Update `src/lib/config.ts`** — Support `RIPMAIL_EMAIL` environment variable (replaces old `IMAP_USER` naming)
...
### Phase 2: Interactive `ripmail wizard`
...
2. **Add `wizard` command to `src/index.ts`**:
   - Route `ripmail wizard` to new wizard implementation
   - Can also support `ripmail setup --wizard` as alias for familiarity
...
```bash
# Agent can now do:
ripmail setup --email user@gmail.com --password "$GMAIL_APP_PASSWORD" --openai-key "$OPENAI_API_KEY" --no-validate

# Or via environment variables:
export RIPMAIL_EMAIL=user@gmail.com
export RIPMAIL_IMAP_PASSWORD="..."
export RIPMAIL_OPENAI_API_KEY="sk-..."
ripmail setup --no-validate
```
...
- Existing config file structure (`~/.ripmail/config.json` and `~/.ripmail/.env`)
...
- All other ripmail commands and functionality
...
- **Backward compatibility:** Should `ripmail setup` without flags still work interactively (calling wizard), or exit with error directing users to `ripmail wizard`?
