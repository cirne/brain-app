...
- Hybrid query latency (`ripmail search "opening eye in bedding calls" --json`) was ~588ms p50.
- Filter-only query latency (`ripmail search "from:hello@theinformation.com" --json`) was ~233ms p50.
...
Keep `ripmail message <id>` and `ripmail thread <id>` as follow-up calls, but align search output to make this easy:
