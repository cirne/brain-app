# OPP-045: iMessage + Unified Messaging Index (Telegram, etc.)
...
**Implication:** A serious cross-channel story needs explicit **handles → person** (or equivalent) and UX for **merge/link**, not only string equality on `from_address`. See [Unified identity across channels](#unified-identity-across-channels-exploration) and [OPP-012](OPP-012-who-smart-address-book.md).
...
Unchanged intent: **`channel`** on `messages` / `threads`, **namespaced** `message_id` or `(channel, external_id)`, **`channel_sync_state`** for non-IMAP cursors, **one FTS** with `channel` where practical, **SCHEMA_VERSION** bump + **re-ingest** story for connector changes. See [AGENTS.md](../../AGENTS.md) early-dev notes on drift and rebuild.
...
**More important after the experiment:** cross-channel **“who is this?”** requires **handles** (phone, email, provider ids) and **optional person merge** — not a single `from_address` string. See [OPP-012](OPP-012-who-smart-address-book.md).
...
- [OPP-016 archived](archive/OPP-016-multi-inbox.md) — multi-mailbox / `mailbox_id`.
- [OPP-044 archived](archive/OPP-044-per-mailbox-sync-and-smtp-config.md) — IMAP sync/SMTP; orthogonal to chat connectors.
- [OPP-012](OPP-012-who-smart-address-book.md) — contacts, merge, `who`.  
- [VISION.md](../VISION.md) — agent-first, local-first product stance.
