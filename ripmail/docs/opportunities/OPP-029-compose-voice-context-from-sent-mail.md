# OPP-029: Compose Voice Context — Sent History and Recipient-Aware Style

**Status:** Not started.

**Relates to:** [OPP-011 archived](archive/OPP-011-send-email.md) (draft + SMTP; remaining “voice profile” bullet), [VISION.md](../VISION.md) (“Voice profile from history”), [OPP-001 archived](archive/OPP-001-personalization.md) (user context for search — complementary personalization surface).
...
4. **What goes in the prompt:**
   ...
   - **Keep distinct from [signatures](archive/OPP-011-send-email.md#key-requirement-configurable-signatures):** signatures remain explicit user-owned config; the model must not invent legal footers or contact blocks.
...
- **No migrations:** Prefer new optional keys in `config.json` and/or a dedicated file under `RIPMAIL_HOME` (see [AGENTS.md](../../AGENTS.md) — schema on DB creation only; voice artifacts are config/files, not new SQLite tables unless justified).
