...
> This document explores how Exa.ai (or a similar neural web search API) could enhance ripmail's search and indexing, specifically focusing on what Exa *inside* ripmail does that an agent calling Exa *separately* cannot.

**Stale diagram (below):** ripmail is **FTS-only** today (no LanceDB/embeddings worker). Use **`ripmail refresh`** wherever this doc says **`ripmail sync`**.

## Core Insight

ripmail has **corpus-level context** the agent doesn't have at query time. The agent knows what it's asking; ripmail knows what's in 500K emails. Exa has **web-scale context** ripmail doesn't have. The integration point is where those two contexts meet.
...
If ripmail ran Exa's content extraction during sync:
...
```
1. Agent calls ripmail: "competitor pricing changes"
2. Agent reads results, identifies the competitor name
3. Agent calls Exa: "Acme Corp pricing 2026"
4. Agent synthesizes both
```

With Exa in ripmail, this collapses to one call:

```
ripmail search "competitor pricing changes" --enriched
...
**What makes this different from the agent doing it:** ripmail extracts the *right* Exa queries from corpus context. The agent only has the user's prompt. ripmail has seen that "competitor" means "Acme Corp" because that's who appears in those email threads. It generates targeted Exa queries the agent wouldn't know to make without first reading the email results.

---

## 3. Contact/Entity Enrichment at Sync Time

Every email has a sender domain (and often a display name). During sync, ripmail could enrich identities so `ripmail who` and contact lookups return web-derived context without an extra agent round-trip. This aligns with the people/contacts work in [OPP-004: People Index and Writable Contacts](opportunities/archive/OPP-004-people-index-contacts.md) — the extended contacts (or people/people_emails) schema is the natural place to store Exa-derived fields.
...
```
  ...
  → ripmail who / ripmail contact show return "Jane Smith, VP Legal at Acme (Series C enterprise SaaS)" in one response
```

**Cost control:** Only enrich identities above a threshold (e.g. min sent_count) or on first `ripmail contact show` (lazy) to limit Exa API usage.

**Why at sync time:** The agent asking "tell me about the person who emailed me about the contract" currently has to: search ripmail → get the sender → call Exa for the company → answer. With pre-enriched contacts, ripmail returns "Jane Smith, VP Legal at Acme Corp (Series C enterprise SaaS company)" in the same response. Zero additional latency.

---

## 4. Similarity from Private Threads

Exa's similarity search takes a URL or text and finds related web content. ripmail has something Exa alone doesn't: private email threads.

```
ripmail similar <thread_id>
...
Use case: an agent working on a deal reads a thread, then asks "what else is out there about this?" ripmail can answer because it has the thread content *and* Exa access. The agent calling Exa separately would need to extract, summarize, and query. ripmail does it in one operation with full context.

---

## 5. When Exa Inside ripmail is NOT Better

- **Pure web search** — agent just wants to search the web. ripmail adds no value wrapping a pass-through.
- **One-off lookups** — "What's Acme's stock price?" No email context to leverage.
- **Agent already has full context** — agent just read 10 emails and knows exactly what to search Exa for. The extra hop through ripmail adds latency.
...
```
ripmail refresh   # (historical docs may say "sync")
...
| **Thread similarity** | ripmail has private content; Exa has the web. Neither alone can bridge both. |

The pattern is always the same: **ripmail has corpus context the agent doesn't, and Exa has web context ripmail doesn't. The value is where those two contexts meet.** Sync-time enrichment amortizes cost; query-time fusion leverages corpus-aware expansion.

---

## See also

- [OPP-004: People Index and Writable Contacts](opportunities/archive/OPP-004-people-index-contacts.md) — `ripmail who`, people index at index time, and `ripmail contact` for writable metadata. Exa contact/entity enrichment (above) is the web-enrichment layer for that same people/contacts model.
