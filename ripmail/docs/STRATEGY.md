...
> This document articulates ripmail's differentiation against API-proxy approaches to agent email access, and why the local-index architecture is a structural moat.
...
**2. Local intelligence layers** — Sync email locally, build a rich index (full-text, semantic, attachment content), and let agents query the local dataset. The provider is the sync source, not the query engine. **This is ripmail.**
...
API proxies are provider-specific. A Gmail API proxy speaks Gmail. It cannot unify Gmail + Outlook + Fastmail + corporate Exchange into one searchable corpus. IMAP is the universal protocol — ripmail speaks it, and can build a unified index across any combination of providers.

---

## Google Workspace CLI (`gws`) vs ripmail

The **[googleworkspace/cli](https://github.com/googleworkspace/cli)** project (`gws`) is the clearest reference implementation of the API-proxy model: **structured JSON**, a **Discovery Service**–driven command surface (Gmail, Drive, Calendar, Sheets, Docs, Chat, Admin, and more), plus `**+` helpers** (e.g. `gmail +triage`, `calendar +agenda`, `drive +upload`). It received significant attention in **March 2026** as a unified Workspace CLI for humans and agents.

**Product status:** The upstream README states that `gws` is **not an officially supported Google product** (Apache-2.0 OSS). Ecosystem and press may describe a “Google Workspace CLI” launch; positioning should not imply a first-party Google product on par with `gcloud`.


| Dimension                            | `**gws`**                                                                                                                                                                                                                                                                                    | **ripmail**                                                                                                                                                                                  |
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Primary job**                      | Call **Workspace HTTP APIs** for mail, Drive, Calendar, productivity apps, and more.                                                                                                                                                                                                         | **IMAP sync** → local store → **query a local index** (FTS5, etc.).                                                                                                                          |
| **Provider scope**                   | **Google / Workspace** (what the APIs expose).                                                                                                                                                                                                                                               | **Any IMAP host**; multi-mailbox in one index.                                                                                                                                               |
| **Query path**                       | Network round-trip; **API quotas** and latency.                                                                                                                                                                                                                                              | After sync, **local** search/read (see [Why API Proxies Are Not Enough](#why-api-proxies-are-not-enough)).                                                                                   |
| **Workspace-native work**            | Strong: Drive files, Sheets, Docs, Calendar, Chat via API — helpers align with those workflows.                                                                                                                                                                                              | **Email** is the focus; not a Sheets/Docs/Calendar control plane.                                                                                                                            |
| **Mail attachments vs native files** | Gmail **attachments** are still API message parts: fetch, download bytes, interpret. **No** mailbox-wide **search inside attachment text** across the corpus unless you build it. Excels at **Google-native** objects (Drive file IDs, Sheets ranges, Doc content, Meet/Calendar artifacts). | **First-class email attachments:** `attachment list` / `attachment read`, **extract** to text (PDF, DOCX, XLSX, HTML, CSV, TXT), optional cache, **indexed with message bodies** for search. |


**Attachments in plain terms:** `gws` is the right tool when the agent’s job is **Workspace APIs** — e.g. open a linked Sheet, append a row, read a Doc by ID, Drive `+upload`. **ripmail** is the right tool when the job is **the mailbox as a dataset**: find and read **email** attachments quickly, including **search inside** extracted attachment text across the synced corpus. Overlap in “read mail” is not overlap in shape: one optimizes for **remote API completeness**, the other for a **local intelligence layer** on the mailbox.

---

## ripmail's Structural Advantages
...
- **Not a replacement for Gmail/Outlook.** Users keep their existing provider. ripmail is an intelligence layer in front of the provider, not a standalone mail client.
- **Not an API proxy.** We don't forward queries to providers. We build a local dataset and query it directly.
- **Outbound mail is optional and gated.** Core value remains deep, fast, intelligent *read* access (search, fetch, summarize). **Send** is available via SMTP send-as-user (`ripmail draft`, `ripmail send`; see [VISION.md](./VISION.md) — "The Full Loop" and [ARCHITECTURE.md](./ARCHITECTURE.md) ADR-024). Optional **RIPMAIL_SEND_TEST=1** restricts recipients for safe dev/test sends.
...
The defensible position is: **ripmail is the intelligence layer between email providers and AI agents**. Providers store and deliver mail. Agents reason and act. ripmail makes the email corpus deeply queryable so agents can reason well.

```
Provider (Gmail, Outlook, Fastmail, ...)
         ↓ IMAP sync
      ripmail (local index: FTS + semantic + attachments)
         ↓ CLI
...
3. **CLI coverage and docs** — Keep search/read/thread/who/attachment/draft/send paths coherent for subprocess agents; document JSON shapes in **AGENTS.md** and per-command `**--help`**.
