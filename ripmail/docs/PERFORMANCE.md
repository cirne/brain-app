# ripmail Performance Metrics

This document summarizes **observed performance** on the platform as of 2025–2026: query latencies, example commands, and response payloads. Data was collected from a live index (~2K messages, Node 20+, macOS) using `ripmail search --timings` and `ripmail who` (JSON includes `_timing.ms`).

**Summary:** Keyword (FTS) and hybrid search consistently complete in **&lt;50ms** in-process. Semantic (hybrid) search adds an embedding API call when the query is not cached; that round-trip dominates total time (~200–400ms). `ripmail who` is in-process only and typically **5–35ms**.
...
```bash
ripmail search "invoice" --fts --timings
```
...
```json
{
  ...
  "totalMatched": 20,
  "returned": 20,
  "hints": ["Tip: Narrow results with from:name or subject:keyword"],
  "timings": {
    "totalMs": 2,
    "ftsMs": 2
  }
...
```bash
ripmail search "invoice" --timings
```
...
```bash
ripmail search "from:stripe receipt" --timings
ripmail search "after:7d subject:meeting" --timings
ripmail search "from:alice@example.com invoice OR receipt" --limit 10
```

**Filter-only (no query text)** — date/metadata only:

```bash
ripmail search "after:7d" --timings --limit 3
```

**Representative timings:** `"totalMs": 2` (no FTS/semantic breakdown when there is no query text).

## 3. `ripmail who` performance and examples

`ripmail who` looks up people by email address or display name and returns sent/received/mentioned counts (and optional enriched fields). It does **not** call an embedding API unless you pass `--enrich`. All timings below are without `--enrich`.
...
```json
{
  ...
  "hints": ["Tip: Use --enrich flag for more accurate name inference and better deduplication (adds ~1-2s latency)"],
  "_timing": { "ms": 35 }
}
```
...
- **`ripmail who`:** **~5–35ms** in-process; no external API unless `--enrich` is used.
...
*Metrics collected 2026-03-07; index ~1,929 messages. Re-run with `ripmail search "<query>" --timings` and `ripmail who "<query>"` (JSON includes `_timing`) to refresh.*
