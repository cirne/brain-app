...
**Current behavior:**
```bash
ripmail search "query"  # Runs both FTS + semantic, combines results
...
- **Current state:** `--mode auto|fts|semantic|hybrid` was implemented in OPP-003, but it's not mentioned in `ripmail search --help`
...
1. **Help text:** Add search mode guidance to `ripmail search --help`: ✅ **FIXED**
   - Added detailed "Search modes" section to `ripmail search --help` with examples
   - Updated main `ripmail --help` to hint at search modes
...
**Fixed 2026-03-06:**
- Enhanced `ripmail search --help` with detailed mode guidance and examples
...
- CLI search usage: `ripmail search "query" [--mode auto|fts|semantic|hybrid]` (see [AGENTS.md](../../AGENTS.md))
