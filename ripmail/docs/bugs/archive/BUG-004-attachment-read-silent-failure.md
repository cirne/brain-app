...
**Reported context:** Agent on macOS (Darwin 25.2.0); task was reading an attachment using numeric index as documented in `ripmail read` output. Reproducibility: Always.

---

## Summary

`ripmail attachment read` fails silently (exit code 1, no stdout/stderr) when given an invalid index such as `0`. The help text says `<index>|<filename>` but does not clarify 0-based vs 1-based indexing. Invalid input should always produce a descriptive error message on stderr.
...
1. Read a message with attachments; output included:
   ```
   To read attachment: ripmail attachment read "<message-id>" <index>|<filename>
   ```
2. Attempted to read attachment by index:
   ```bash
   ripmail attachment read "<message-id>" 0
   ```
...
```bash
ripmail attachment read "<message-id>" "filename.xlsx"
```
...
- **Tested with:** `ripmail attachment read "<CH3PR84MB38746104F19C2113B72E38DBFD7AA@CH3PR84MB3874.NAMPRD84.PROD.OUTLOOK.COM>" 0` and `ripmail attachment read "..." 1`
