# BUG-009: `ripmail wizard` Crashes with Stack Trace on Non-Interactive Stdin — Agent-Reported
...
**Reported context:** Agent on macOS (Darwin 25.3.0); ripmail config: none (fresh install, no `~/.ripmail/`). Reproducibility: Always (any non-TTY stdin).

---

## Summary

Agent (or script) invokes `ripmail wizard` in a non-TTY environment. The interactive prompt library throws an unhandled `ExitPromptError` and prints a full Node.js stack trace. Agents will typically hit this since they call commands via subprocess with piped stdio.

---

## What the agent did (and what happened)

1. Ran `ripmail wizard` with piped or closed stdin (any non-TTY context):
   ```bash
   echo "" | ripmail wizard
   ```
2. **Expected:** Graceful error message, e.g.:
   ```
   Wizard requires an interactive terminal. Use 'ripmail setup' instead.
   ```
   Exit code 1, no stack trace.
3. **Actual:** Unhandled `ExitPromptError` with full stack trace:
   ```
   file:///Users/cirne/dev/ripmail/node_modules/@inquirer/core/dist/lib/create-prompt.js:64
             reject(new ExitPromptError(`User force closed the prompt with ${code} ${signal}`));
                    ^

   ExitPromptError: User force closed the prompt with 0 null
       at file:///Users/cirne/dev/ripmail/node_modules/@inquirer/core/dist/lib/create-prompt.js:64:20
       at Emitter.emit (/Users/cirne/dev/ripmail/node_modules/signal-exit/src/index.ts:108:13)
       at SignalExit.#processEmit (/Users/cirne/dev/ripmail/node_modules/signal-exit/src/index.ts:304:21)
       at process.#process.emit (/Users/cirne/dev/ripmail/node_modules/signal-exit/src/index.ts:248:31)
...
- The `--help` output correctly suggests `ripmail wizard` for interactive use and `ripmail setup` for CLI/agent use. The issue is just missing a guard at the start of `wizard` to detect non-TTY stdin.
