...
1. Ran `ripmail who "matt shandera"` (without `--all` flag).
...
- **Status:** Verified
- **Date:** 2026-03-07
- **Result:** Fix confirmed. `ripmail who "matt shandera"` now returns only the real Matt Shandera entry. The `drive-shares-dm-noreply@google.com` address no longer leaks through the noreply filter.
- **Tested with:** `ripmail who "matt shandera"`
