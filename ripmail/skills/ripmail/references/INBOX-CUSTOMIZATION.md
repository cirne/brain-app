...
**Thread scope:** Each rule may set **`threadScope`** (default **`true`**). When **`true`**, matching **one** message assigns that rule to **every** other message in the same conversation (**same `thread_id` in SQLite**) that is still **`pending`** within the inbox scan window—so recurring threads (e.g. tee-time back-and-forth) are triaged as a unit. Use **`"threadScope": false`** or **`ripmail rules add … --message-only`** / **`ripmail rules edit <id> --message-only`** for **message-only** matching; **`ripmail rules edit <id> --whole-thread`** sets thread scope back on. **`ripmail rules list`** / **`show`** print JSON that includes **`threadScope`** on each rule.
