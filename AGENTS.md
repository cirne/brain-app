...
**OPP-048 (local feedback / issues):** the same `BRAIN_EMBED_MASTER_KEY` is accepted as a **Bearer** token for `GET /api/issues` and `GET /api/issues/:id` in **single-tenant** mode only (loopback / operator tooling, not a public-internet or untrusted-LAN story). In **multi-tenant** mode, use a normal `brain_session` to read issues. `POST /api/issues/draft`, `POST /api/issues/submit`, and the `product_feedback` agent tool (draft + `confirmed` submit) require an unlocked vault. `npm run issues:list` and `npm run issues:fetch` call those GET routes with the env key.
