...
Two tasks in [`tasks/wiki-v1.jsonl`](tasks/wiki-v1.jsonl): **`buildout`** uses [`getOrCreateWikiBuildoutAgent`](../src/server/agent/wikiBuildoutAgent.ts) (**`edit`** + mail/web tools + `search_index`; **`write`** cannot create new pages — same deepen-only rules as Your Wiki enrich, [archived OPP-067](../docs/opportunities/archive/OPP-067-wiki-buildout-agent-no-new-pages.md)); **`cleanup`** uses [`createCleanupAgent`](../src/server/agent/agentFactory.ts) (read / grep / find / `edit` — no `write`).
