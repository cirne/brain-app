...
describe('aggregateSyncErrors', () => {
  ...
  it('returns empty when all succeed', () => {
    expect(aggregateSyncErrors(ok, ok)).toEqual([])
  })

  it('collects rejection reasons in Docs / Inbox order', () => {
    ...
    expect(aggregateSyncErrors(wikiRej, ok)).toEqual(['Docs: net'])
    expect(aggregateSyncErrors(ok, inboxRej)).toEqual(['Inbox: timeout'])
  })
  ...
  it('uses json error or default when ok is false', () => {
    ...
    expect(aggregateSyncErrors(wikiBad, ok)).toEqual(['Docs: wiki unavailable'])
    const noMsg: PromiseFulfilledResult<SyncResponseBody> = {
      status: 'fulfilled',
      value: { ok: false },
    }
    expect(aggregateSyncErrors(noMsg, ok)).toEqual(['Docs: sync failed'])
  })

  it('does not error when fulfilled value is undefined', () => {
    const empty: PromiseFulfilledResult<SyncResponseBody> = {
      status: 'fulfilled',
      value: undefined as unknown as SyncResponseBody,
    }
    expect(aggregateSyncErrors(empty, ok)).toEqual([])
  })
})

describe('runParallelSyncs', () => {
  it('calls wiki and inbox sync endpoints and aggregates', async () => {
