<script lang="ts">
  ...
  import DayEvents, { type CalendarEvent } from '@tw-components/DayEvents.svelte'
  import CalendarEventDetail from '@tw-components/CalendarEventDetail.svelte'
      ...
      {#each days as { date, ymd } (ymd)}
        ...
        >
          <div class="day-header mb-1.5 flex items-center gap-2">
            ...
            {#if ymd === todayYMD}
              <span
                class="today-badge rounded-[10px] bg-accent px-1.5 py-px text-[10px] font-semibold uppercase tracking-[0.04em] text-white"
              >today</span>
            {/if}
          </div>
          <DayEvents
