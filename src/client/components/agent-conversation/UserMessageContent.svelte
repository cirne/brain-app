  ...
  {#each segments as seg, i (i)}
    {#if seg.kind === 'text'}{seg.text}{:else if seg.kind === 'wiki'}<span
      ...
      ><WikiFileName path={seg.path} /></span>{:else}<span
        class="user-mention user-mention--person inline-flex items-baseline gap-0.5 align-baseline rounded-full bg-[color-mix(in_srgb,var(--accent,#2563eb)_14%,transparent)] px-1.5 py-0 text-[0.9em] font-medium text-accent"
        title={`@${seg.handle}`}
