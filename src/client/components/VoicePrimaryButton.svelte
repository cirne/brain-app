<script lang="ts">
  ...
  let {
    ...
    holdGated = false,
    pulseKey = 0,
    /** Match {@link AgentInput} send rail inside `input-shell` (unified composer voice mode). */
...
<div
  class={cn(
    embeddedInComposer
      ? 'voice-primary-hit flex shrink-0 flex-row items-stretch self-stretch'
      : 'voice-primary-hit box-border flex h-full min-w-[33%] items-center justify-end [padding-right:max(12px,env(safe-area-inset-right,0px))]',
  )}
>
  <button
    type="button"
    class={cn(
      embeddedInComposer
        ? cn(sendBtnBase, phase === 'transcribing' && 'opacity-70')
        : cn(
  ...
  >
    ...
    <span
      class={cn(
        'voice-primary-icons relative grid h-full w-full place-items-center',
        embeddedInComposer ? 'text-accent-foreground' : 'text-white',
      )}
    >
      {#if phase === 'transcribing'}
        <span
          class="voice-primary-icon-layer voice-primary-icon-layer--show grid place-items-center opacity-100 transition-opacity duration-150 [grid-area:1/1]"
        >
          <Loader2 size={embeddedInComposer ? 20 : 26} strokeWidth={2} aria-hidden="true" />
        </span>
      {:else}
        <span
          class={cn(
            'voice-primary-icon-layer grid place-items-center opacity-0 transition-opacity duration-150 [grid-area:1/1]',
            (phase === 'idle' || phase === 'arming') && 'voice-primary-icon-layer--show opacity-100',
          )}
        >
          <Mic size={embeddedInComposer ? 20 : 26} strokeWidth={2.25} aria-hidden="true" />
        </span>
        <span
          class={cn(
            'voice-primary-icon-layer voice-primary-send-wrap grid place-items-center opacity-0 transition-opacity duration-150 [grid-area:1/1] [&_svg]:block',
            phase === 'recording' && 'voice-primary-icon-layer--show opacity-100',
          )}
        >
          <ArrowUp size={embeddedInComposer ? 20 : 22} strokeWidth={2.5} aria-hidden="true" />
        </span>
      {/if}
    </span>
  </button>
...
<style>
  /* Pseudo-element animation for the active pulse ring (dock layouts only). */
