...
describe('buildBaseSystemPrompt', () => {
  it('prefers wiki lookup before mail for general questions', async () => {
    ...
    expect(s).toContain('**Start with the wiki**')
    expect(s).toContain('**Start with sources**')
    expect(s).toMatch(/wiki.*orientation/i)
    expect(s).toMatch(/not a stopping rule/i)
    expect(s).toContain('only as accurate as it has been kept')
    expect(s).toMatch(/newest dated source normally wins/i)
    expect(s).toContain('search_index')
    expect(s).toContain('read_mail_message')
  ...
  it('steers chat wiki toward narrow capture and Chat capture provenance (OPP-066)', async () => {
    ...
    expect(s).toMatch(/deepens thin pages over time/i)
    expect(s).not.toContain('WikiBuilder')
    expect(s).not.toContain('wiki-edits.jsonl')
  })

  it('mentions suggest_reply_options and bans bracket placeholder text', async () => {
    const { buildBaseSystemPrompt } = await import('./assistantAgent.js')
    const s = buildBaseSystemPrompt(false, wiki)
    expect(s).not.toContain('[suggest_reply_options]')
    expect(s).toContain('**`suggest_reply_options`**')
    expect(s).toContain('chip labels match your answer')
