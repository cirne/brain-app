...
export function createRipmailAgentTools(wikiDir: string) {
  ...
  const readIndexedFile = defineTool({
    ...
    label: 'Read file',
    description:
      '**Ripmail indexed files only (not the wiki `read` tool).** Read body text for **Google Drive** or **localDir** items indexed into ripmail: pass **`messageId`** from **`search_index`** (same JSON field as mail — for Drive it is the remote file id, usually long alphanumeric). Also use for **allowed absolute filesystem paths** (tilde ok) when you need extracted text/PDF/etc. **Google Drive:** full reads call Drive metadata each time, then reuse an on-disk export cache when the remote content fingerprint matches (same logic as index sync). **localDir:** reads from local files / extracted text. Never call **`read_attachment`** for these.',
    parameters: Type.Object({
      id: Type.String({
        description:
  ...
  const draftEmail = defineTool({
    name: 'draft_email',
    description:
      'Create an email draft for **regular mailbox mail**. action=new composes a fresh email (requires **to**, **subject**, and **body**). action=reply drafts a reply (requires **message_id** and **body**; subject defaults to Re: original unless you pass **subject**). For **action=reply** only, **body** is **your new reply text** — the app **appends** quoted plaintext for **each message in the mail thread** (same thread id) from oldest through the message you reply to, with standard `On … wrote:` / `>` quoting (same idea as desktop mail apps: full thread through that message); **do not** paste or recreate those bodies in **body**. action=forward forwards an existing message (requires **message_id**, **to**, and **body**). For **action=forward** only, **body** is **your new cover note** — the app **appends** a standard forwarded-message block (headers + original body from the index); **do not** paste the original message in **body**. For reply/forward, **message_id** must be the exact `messageId` from `list_inbox`, `search_index`, or `read_mail_message`; the tool fails if the message is not in the index, so re-search instead of guessing. Replies default to **reply-all** recipients from the source message (sender + relevant To/Cc, excluding your mailbox and duplicates); pass **reply_all: false** to force sender-only. **to** must be a valid email or a name/handle the index can resolve to one contact; otherwise the tool errors with a hint to use **find_person** first. For **action=new**, **body** is the full outgoing text (no quoted original). If the user **@mentions** a collaborator and wants the **Ask Brain** path, use **ask_collaborator** when that tool is available. When the workspace has more than one Gmail account, optional **from** picks the send mailbox. For **Braintunnel collaborator** email set **b2b_query: true** so the subject line is normalized to include the `[braintunnel]` marker (after any Re:/Fwd:); you still choose the substantive subject text. **grant_id** (`bqg_…`) is assistant-only validation context — never put it in **body** unless the user asks. Returns draft id, recipients, and subject; **do not repeat the full body in chat** unless the user asks.',
    label: 'Draft Email',
    parameters: Type.Object({
      action: Type.Union([Type.Literal('new'), Type.Literal('reply'), Type.Literal('forward')], { description: '"new" | "reply" | "forward"' }),
      body: Type.String({
        description:
          'For action=new: full message body. For action=reply: **new reply text only** — quoted thread is appended from the index. For action=forward: **cover note only** — forwarded original is appended from the index.',
      }),
  ...
  const editDraft = defineTool({
    ...
    async execute(_toolCallId: string, params: {
      ...
      const home = ripmailHomeForBrain()
      await ripmailDraftEdit(home, params.draft_id, {
        body: params.body,
        subject: params.subject,
        to: params.to,
        cc: params.cc,
  ...
  const findPerson = defineTool({
    ...
    label: 'Find Person',
    description:
      'Resolve identity and correspondence context from indexed mail contacts (`ripmail who`) and wiki markdown—mailbox-native names/emails/org cues, not a substitute for full-body mail search. Prefer this when the question is who someone is (or which email/company applies), or when `search_index` returns many hits or ambiguous snippets for a person- or company-heavy query—confirm identity before treating From headers or the first hit as authoritative. Empty query lists top contacts by frequency (`ripmail who --limit 60`). Otherwise pass a name fragment, email, or phone (any format; phones match flexibly in wiki files).',
    parameters: Type.Object({
      query: Type.String({
        description:
          'Empty or whitespace: top contacts by frequency (`ripmail who --limit 60`). Otherwise: person name or partial name, email address, or phone number (flexible formatting).',
      }),
    }),
