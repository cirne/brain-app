...
process environment; use --provider / --model to set BRAIN_LLM for this run.
  ...
  3. B2B v1 — Kean asks Lay's brain through a seeded grant → data-eval/eval-runs/b2b-v1-*.json
...
async function main(): Promise<void> {
  const [{ runEnronV1Main }, { runMailComposeV1Main }, { runB2BV1Main }, { runWikiV1Main }] = await Promise.all([
  ...
  const nB2B = await runB2BV1Main()
  const nWiki = await runWikiV1Main()
  const onlyId = process.env.EVAL_CASE_ID?.trim()
  if (onlyId && nEnron + nCompose + nB2B + nWiki === 0) {
    console.error(
      `[eval:jsonl] No task with id ${JSON.stringify(onlyId)} in Enron, mail-compose, B2B, or wiki JSONL task files (check eval/tasks/enron-v1.jsonl, mail-compose-v1.jsonl, b2b-v1.jsonl, wiki-v1.jsonl ⊕ wiki-kean-v1.jsonl default bundle, or EVAL_*_TASKS overrides).`,
    )
    process.exit(1)
  }
}
