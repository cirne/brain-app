...
process environment; use --provider / --model to set BRAIN_LLM for this run.
  ...
  3. Wiki v1 — buildout + cleanup (wiki-v1.jsonl ⊕ wiki-kean-v1.jsonl by default; override with EVAL_WIKI_TASKS) → data-eval/eval-runs/wiki-v1-*.json
...
async function main(): Promise<void> {
  const [{ runEnronV1Main }, { runMailComposeV1Main }, { runWikiV1Main }] = await Promise.all([
  ...
  if (onlyId && nEnron + nCompose + nWiki === 0) {
    console.error(
      `[eval:jsonl] No task with id ${JSON.stringify(onlyId)} in Enron, mail-compose, or wiki JSONL task files (check eval/tasks/enron-v1.jsonl, mail-compose-v1.jsonl, wiki-v1.jsonl ⊕ wiki-kean-v1.jsonl default bundle, or EVAL_*_TASKS overrides).`,
    )
    process.exit(1)
  }
}
