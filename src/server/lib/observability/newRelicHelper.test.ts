...
describe('newRelicHelper', () => {
  ...
  afterEach(async () => {
    const { releaseAllPendingToolCallSegments } = await import('@server/lib/observability/newRelicHelper.js')
    releaseAllPendingToolCallSegments()
    delete process.env.NEW_RELIC_LICENSE_KEY
  })

  it('sanitizeForNewRelicJson redacts sensitive keys', async () => {
    const { sanitizeForNewRelicJson } = await import('@server/lib/observability/newRelicHelper.js')
  ...
  it('recordToolCallEnd uses explicit workspaceHandle and backgroundRunId for wiki', async () => {
    const { recordToolCallStart, recordToolCallEnd } = await import('@server/lib/observability/newRelicHelper.js')
  ...
  it('resultSizeBucketFromCharCount returns bounded bands', async () => {
    const { resultSizeBucketFromCharCount } = await import('@server/lib/observability/newRelicHelper.js')
  ...
  it('recordLlmCompletionsForTurn and recordLlmAgentTurn emit', async () => {
    const { recordLlmCompletionsForTurn, recordLlmAgentTurn } = await import('@server/lib/observability/newRelicHelper.js')
    const a = {
  ...
  it('setAgentTurnTransactionAttribute forwards agentTurnId to addCustomAttribute', async () => {
    const { setAgentTurnTransactionAttribute } = await import('@server/lib/observability/newRelicHelper.js')
  ...
  it('addTransactionCustomAttributes namespaces keys and skips nullish', async () => {
    const { addTransactionCustomAttributes } = await import('@server/lib/observability/newRelicHelper.js')
  ...
  it('resolveNewRelicEndUserId maps tenant sentinel and usr ids', async () => {
    const { resolveNewRelicEndUserId } = await import('@server/lib/observability/newRelicHelper.js')
    const usr =
  ...
  it('applyBrainTenantContextToNewRelicTransaction sets attributes and setUserID for usr tenant', async () => {
    const { applyBrainTenantContextToNewRelicTransaction } = await import('@server/lib/observability/newRelicHelper.js')
