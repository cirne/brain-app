 ...
 *
 * Used by: agent **`read_mail_message`** / **`read_indexed_file`**, GET /api/files/read, GET /api/inbox/:id (`ripmail read --json --full-body` + `bodyHtml` when present; `--full-body` turns off 500k-char cap on `read <path>` local files).
 */
