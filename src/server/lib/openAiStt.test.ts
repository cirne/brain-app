...
describe('openAiStt', () => {
  ...
  it('isOpenAiSttConfigured is false when key missing', async () => {
    const { isOpenAiSttConfigured } = await import('./openAiStt.js')
    delete process.env.OPENAI_API_KEY
  ...
  it('openAiSttModel defaults to whisper-1', async () => {
    const { openAiSttModel } = await import('./openAiStt.js')
  ...
  it('transcribeOpenAiStt calls transcriptions.create and returns trimmed text', async () => {
    const { transcribeOpenAiStt } = await import('./openAiStt.js')
  ...
  it('transcribeOpenAiStt throws when over max bytes', async () => {
    const { OPENAI_STT_MAX_BYTES, transcribeOpenAiStt } = await import('./openAiStt.js')
