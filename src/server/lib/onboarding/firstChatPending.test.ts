...
describe('firstChatPending', () => {
  it('tryConsumeFirstChatPending returns false when no file', async () => {
    const { tryConsumeFirstChatPending } = await import('@server/lib/onboarding/firstChatPending.js')
    expect(await tryConsumeFirstChatPending()).toBe(false)
  })

  it('write then consume returns true once', async () => {
    const { writeFirstChatPending, tryConsumeFirstChatPending } = await import('@server/lib/onboarding/firstChatPending.js')
  ...
  it('hasFirstChatPending is true iff file exists', async () => {
    const { hasFirstChatPending, writeFirstChatPending, tryConsumeFirstChatPending } = await import('@server/lib/onboarding/firstChatPending.js')
