...
describe('assertManageSourcePathNotInsideSiblingTenant', () => {
  ...
  it('throws when path is inside another tenant directory', async () => {
    ...
    const { runWithTenantContext } = await import('@server/lib/tenant/tenantContext.js')
    const { assertManageSourcePathNotInsideSiblingTenant } = await import('@server/lib/chat/agentPathPolicy.js')
