...
export function rewriteOpenWikiTargetForUnifiedTree(
  ...
  const toolsRoot = wikiToolsRootAbs
  const personalAbs = join(toolsRoot, 'me', rel)
...
export function rewriteOpenToolArgsIfNeeded(wikiToolsRootAbs: string, args: unknown): unknown {
  if (args == null || typeof args !== 'object' || !('target' in args)) return args
  const a = args as { target?: unknown }
  const newTarget = rewriteOpenWikiTargetForUnifiedTree(wikiToolsRootAbs, a.target)
