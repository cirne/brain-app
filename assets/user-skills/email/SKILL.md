...
5. **Draft** — **`draft_email`** / **`edit_draft`**; respect tone from **`me.md`** and sent mail context when inferable (**[references/COMPOSE.md](references/COMPOSE.md)**). For replies, default to reply-all unless the user asks for sender-only. If recipient identity is unclear ("email Fred"), use **`find_person`** (and `ripmail who` data) before drafting instead of guessing addresses.
