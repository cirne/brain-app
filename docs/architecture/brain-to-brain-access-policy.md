...
**Status:** Draft — **design synthesis** for collaboration trust surfaces (brain-query, future bilateral flows, wiki shares). **Phase 0 shipped behavior** is described in [brain-query-delegation.md](./brain-query-delegation.md) and **[braintunnel-b2b-chat.md](./braintunnel-b2b-chat.md)** (chat-native tunnels + grants); this doc is the **target architecture** for evolving policy without turning admin into OAuth-scale complexity.
...
**Policy text + LLM privacy filtering** (Spike 0) can **auto-send** an answer when the pipeline completes. **Shipped (chat-native):** per-connection **review** vs **auto** is enforced in **Braintunnel B2B** — owner **review queue**, **`POST /api/chat/b2b/approve`**, and durable **`b2b_inbound_query`** / tunnel notifications in tenant SQLite — see **[braintunnel-b2b-chat.md](./braintunnel-b2b-chat.md)**. Mail **`notify`** / Ripmail remains a **separate** input to the **same** `notifications` substrate, not the transport for brain-to-brain Q&A.

**Product evolution:** even with tunnel **review/approve**, a **unified anticipatory brief** (one ranked strip across mail, calendar, wiki, and tunnels) remains **[IDEA: Anticipatory assistant brief](../ideas/IDEA-anticipatory-assistant-brief.md)** backlog — complementary to chat rails, not a substitute for the shipped approval path.
