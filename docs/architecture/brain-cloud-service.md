...
For features like [OPP-008](../opportunities/OPP-008-tunnel-qr-phone-access.md) (phone access via QR / tunnel) and the brain-to-brain direct channels in OPP-001, there may be infrastructure we need to run centrally for NAT traversal and tunnel setup — especially when both brains are behind NAT and can't reach each other directly.
...
- **Brain-to-brain relay (ephemeral).** During the OPP-001 handshake, before two brains have established a direct channel, a central relay might broker the initial exchange. After the channel is set up, the relay is no longer involved.
