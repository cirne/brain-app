...
3. **Authoritative read**  
   When the agent selects a hit, **fetch current body from the provider** (HTTP export, blocks API, etc.). For **Google Drive**, satisfy from disk cache under `ripmail/<source-id>/cache/read-body/` when a **`drive.files.get` metadata fingerprint** matches the fingerprint stored beside the cached export (`driveFileFingerprint` in [`googleDriveFileFingerprint.ts`](../../src/server/ripmail/sync/googleDriveFileFingerprint.ts)), identical to sync/index hashing—otherwise re-export and rewrite cache.
...
**Implementation note:** persistent files under `ripmail/<source-id>/cache/` survive restarts; Drive **read-body** entries stay valid until **remote fingerprint changes**, without time-based eviction.
