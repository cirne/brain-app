...
| *(Brain ignores)* `RIPMAIL_HOME` | — | **Not a Braintunnel knob.** Ripmail storage is **`$BRAIN_HOME/<ripmail>/`** from layout ([`shared/brain-layout.json`](../../shared/brain-layout.json)); spawned ripmail children get a computed `RIPMAIL_HOME`. Multi-tenant: **`$tenantHome/<ripmail>/`**. Standalone **`ripmail`** CLI may still use `RIPMAIL_HOME` (see ripmail docs). |
