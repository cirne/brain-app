...
| `RIPMAIL_BIN` | _(unset)_ | **Rare.** Only subprocess-backed tests / legacy CLI adapters (`execRipmailArgv`, `ripmail clean`). Normal mail uses in-process `@server/ripmail`. |
| *(Brain ignores)* `RIPMAIL_HOME` | — | **Not a Braintunnel knob.** Ripmail storage is **`$BRAIN_HOME/<ripmail>/`** from layout ([`shared/brain-layout.json`](../../shared/brain-layout.json)); the server uses a computed home per tenant in MT mode. Multi-tenant: **`$tenantHome/<ripmail>/`**. |
