...
**Future:** Optional **multi-user sharing** of subtrees (invited collaborator, RO/RW) is an active product idea only—see [IDEA: Brain-to-brain collaboration](../ideas/IDEA-wiki-sharing-collaborators.md), [OPP-064](../opportunities/OPP-064-wiki-directory-sharing-read-only-collaborators.md), and **[wiki-directory-sharing.md](./wiki-directory-sharing.md)** (policy in each brain home, server-enforced reads, optional grantee symlinks). Implementation must preserve explicit scope and tenancy rules in [PRODUCTIZATION](../PRODUCTIZATION.md).
