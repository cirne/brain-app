...
**Status:** **Implemented (core, 2026-04-29).** The post-chat async job, delta-anchored cleanup (`changedFiles`), supervisor handoff, chat header affordance, status API, and client polling (debounced bootstrap + burst) are in tree. **Not fully implemented** relative to the whole document: optional product controls, overlap guards, richer telemetry dashboards, and wiki eval fixtures remain **deferred** (see **Implementation status** below).
...
Archived **background maintenance** ([OPP-015](./archive/OPP-015-wiki-background-maintenance-agents.md)) was folded into Your Wiki; this proposal adds **delta-anchored cleanup input** everywhere lint runs, plus **higher-frequency** follow-up on chat writes.
...
**Placement:** In the **main chat** header / toolbar—**immediately left of** the existing **sound on/off** control (and the trash / clear control)—see [`AgentChat.svelte`](../../src/client/components/AgentChat.svelte) (`wiki-polish-slot`, Lucide `Sparkles`).
...
**Lifecycle + data:** [`GET /api/chat/wiki-touch-up/:sessionId`](../../src/server/routes/chat.ts) reads the stable `BackgroundRunDoc` (`wiki-touch-up-<sessionId>`) from [`wikiTouchUpJob.ts`](../../src/server/lib/chat/wikiTouchUpJob.ts). The client uses a **debounced** one-shot fetch on session change plus a **short burst** poll after a wiki-mutating turn (not SSE on the main transcript).
...
| Main-chat path tracking | **Shipped** | [`streamAgentSse.ts`](../../src/server/lib/chat/streamAgentSse.ts) + handlers collect wiki tool paths per turn. |
| Async post-chat job | **Shipped** | [`wikiTouchUpJob.ts`](../../src/server/lib/chat/wikiTouchUpJob.ts): **400ms** server debounce/merge, `attachRunTrackerSource: 'wikiTouchUp'`. |
| No recursive enqueue from touch-up | **Shipped** | Touch-up runs outside main-chat SSE; enqueue only from [`streamAgentSse.ts`](../../src/server/lib/chat/streamAgentSse.ts) `finally` (main-chat turn scope); cleanup uses separate runner. |
| Chat header + polling | **Shipped** | [`AgentChat.svelte`](../../src/client/components/AgentChat.svelte); [`chat.ts`](../../src/server/routes/chat.ts) GET route. Client: debounced bootstrap + burst interval. |
...
| Telemetry (completion rate, p50 duration, dashboards) | **Partial** | `wikiTouchUp` source in NR tool plumbing; dedicated touch-up KPIs not spelled out as dashboards. |
...
- **Eval hook:** Extend wiki/agent evals ([wiki-and-agent-evaluation.md](../wiki-and-agent-evaluation.md)) with fixtures where the main turn introduces a bad link or stale time; touch-up pass **fixes or flags** per gold criteria.

## Related

- [OPP-033](./OPP-033-wiki-compounding-karpathy-alignment.md) — Your Wiki supervisor; enrich → cleanup uses the **same** `changedFiles`-anchored cleanup invocation as post-chat touch-up (supervisor typically **larger** `changedFiles`, **higher** budget).
