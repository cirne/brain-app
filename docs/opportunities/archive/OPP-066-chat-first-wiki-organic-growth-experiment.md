...
**Status:** Partially shipped (2026-04-30); **archived** — see banner above. Core direction landed in `main`:
...
- **Cleanup agent (OPP-062, retired 2026-04-30):** historically ran structural maintenance shortly after **vault-mutating** chat turns; **superseded** by supervisor lap cleanup + **`wiki-edits.jsonl`** strategy — see [archived OPP-062](./OPP-062-post-turn-wiki-touch-up-agent.md), [OPP-033](../OPP-033-wiki-compounding-karpathy-alignment.md).
...
- **Maintenance stack stays:** supervisor enrich → cleanup; ~~post-turn touch-up (OPP-062)~~ **retired 2026-04-30** — delta-anchored lint on **lap** cadence remains; planned log-derived cleanup anchors ([OPP-033](../OPP-033-wiki-compounding-karpathy-alignment.md)).
...
After onboarding, the `Your Wiki` supervisor launches an enrich → cleanup loop that processes the user's full inbox in background laps. The chat agent answers questions wiki-first then mail, and is loosely directed to "write or update the wiki when you learn something durable." ~~Post-turn touch-up (OPP-062)~~ Structural cleanup for chat-authored pages is **lap-scoped**; see [archive/OPP-062](./OPP-062-post-turn-wiki-touch-up-agent.md) and **[Lint inputs and cadence (2026)**](../OPP-033-wiki-compounding-karpathy-alignment.md#lint-inputs-and-cadence-2026).
...
- **Delta-anchored cleanup**: runs after **each enrich lap** in the supervisor loop, anchored on **`changedFiles` from enrich** (~cheap and targeted). **Post-turn touch-up (OPP-062) retired 2026-04-30** — extending cleanup anchors from **`wiki-edits.jsonl`** is **planned** ([OPP-033](../OPP-033-wiki-compounding-karpathy-alignment.md)).
...
These remain the maintenance shell for the organic model; **immediate** chat-turn polish was intentionally removed **2026-04-30** in favor of supervisor cadence plus the structured edit log ([archived OPP-062](./OPP-062-post-turn-wiki-touch-up-agent.md)).

The experiment remains about **what drives page creation** (chat-triggered synthesis rather than background batch discovery), while **lap-scoped lint** continues to converge with **`wiki-edits.jsonl`**-driven anchors.
...
- [OPP-062](./OPP-062-post-turn-wiki-touch-up-agent.md) — Post-turn touch-up (**shipped then retired 2026-04-30**); superseded direction in [OPP-033](../OPP-033-wiki-compounding-karpathy-alignment.md).
