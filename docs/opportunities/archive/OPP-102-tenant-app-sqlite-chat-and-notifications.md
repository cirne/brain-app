...
- Per-tenant **`var/brain-tenant.sqlite`** (`files.tenantSqlite` in [`brain-layout.json`](../../../shared/brain-layout.json)) with **`chat_sessions`**, **`chat_messages`**, and **`notifications`**.
...
**See also:** [chat-history-sqlite.md](../../architecture/chat-history-sqlite.md) · [per-tenant-storage-defense.md](../../architecture/per-tenant-storage-defense.md) · [data-and-sync.md](../../architecture/data-and-sync.md) · [brain-to-brain-access-policy.md § notification](../../architecture/brain-to-brain-access-policy.md#notification-inbox-and-human-in-the-loop-prerequisite-for-secure-brain-to-brain) · **[IDEA: Anticipatory assistant brief](../../ideas/IDEA-anticipatory-assistant-brief.md)** · **[IDEA: Brain-query delegation](../../ideas/IDEA-brain-query-delegation.md)** · [brain-layout.json](../../../shared/brain-layout.json) / [`brainLayout.ts`](../../../src/server/lib/platform/brainLayout.ts)
...
**Decision:** **Ripmail remains authoritative** for inbox rules and `inbox_decisions`. **`notify`** decisions are **mirrored** into the tenant **`notifications`** table after each successful `ripmail refresh` / `backfill` (see `syncMailNotifyNotifications.ts`): `source_kind = mail_notify`, dedupe via **`idempotency_key = mail_notify:<message_id>`**. **`inform`** / **`ignore`** do not create app notifications. Mail bodies stay in ripmail/maildir only.
...
2. **Schema lifecycle:** bumping **`TENANT_SCHEMA_VERSION`** in code **recreates** DB without manual steps; documented for operators/devs.
...
4. **Layout:** tenant path documented in `brain-layout` (or sibling doc) so desktop + hosted use the same relative path. **Implemented:** `files.tenantSqlite` → `<tenant>/var/brain-tenant.sqlite` (see [`brain-layout.json`](../../../shared/brain-layout.json)).

## Related

- [chat-history-sqlite.md](../../architecture/chat-history-sqlite.md) — session schema; status line points here.
