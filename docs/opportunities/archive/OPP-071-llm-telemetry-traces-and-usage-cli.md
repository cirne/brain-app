# Archived: OPP-071 — LLM telemetry
...
| **Local CLI** | `npm` script to summarize usage from **`var/brain-tenant.sqlite`** (chat messages) and `background/runs/*.json` under `$BRAIN_HOME`. |
...
- **`chat_messages`** in **`var/brain-tenant.sqlite`** — for each session, sum `content_json` assistant `usage` (see [`chatStorage.ts`](../../../src/server/lib/chat/chatStorage.ts)).
...
- [pi-agent-stack.md](../../architecture/pi-agent-stack.md) — `Agent` events, `usage` origin
