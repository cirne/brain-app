...
**Status:** In progress — **Phase 0 backend + `ask_brain` shipped**; Hub Sharing surface and chat `@` mentions for **people + wiki** landed (this OPP); fragment / hard-predicate work still deferred to **Spikes 3–4**.  
**Related:** [IDEA brain-query delegation](../ideas/IDEA-brain-query-delegation.md) · [architecture: brain-query-delegation](../architecture/brain-query-delegation.md) · **[architecture: brain-to-brain access policy](../architecture/brain-to-brain-access-policy.md)** (**Spike 1** Hub/B2B UI here first; fragments / layer 2 = **Spikes 3–4** in that doc)
...
- **Chat (done):** Users can `@`-mention **confirmed workspace handles** from the composer; **person mentions render differently** from **wiki document** mentions in the transcript. Grammar documented in the **Disambiguation grammar** table above.
