...
- **Brain Hub** (`/hub`): Accessible via the top-nav widget. Consolidates wiki stats, recent docs, background agent activity, and quick links (profile, email status, sync).
- **Left nav** (`ChatHistory`): “New chat,” then **Chats** and **Recents** (docs/email).
- **Top bar**: brand, global search (⌘K), and the **Brain Hub widget** (replaces legacy standalone sync/expansion buttons). Configuration is **explicit** (Brain Hub, env, `BRAIN_HOME`, ripmail config) or **conversational** (agent tools).
...
3. **Scaling:** Without a named “Settings” shell (now Brain Hub), each new capability risks a **new modal, menu, or buried route**—inconsistent UX and higher support cost.
...
1. User clicks the **Brain Hub widget** in the top nav.
2. The **Brain Hub** opens at `/hub`, replacing the main chat area while keeping the sidebar and top nav.
3. User completes tasks such as: show **QR / URL** for phone access (future), **add or remove** indexed folders (future), see **mail account / OAuth** status, adjust **default agent tone** or **custom instructions** (future), manage **skills** shortcuts, or export/diagnostics—depending on what’s shipped.

## What belongs in Brain Hub vs in chat


| Kind of change                               | Prefer Brain Hub GUI                                                 | Prefer chat / tools / files                                     |
| -------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------------------------- |
...
| Experimental flags                           | Brain Hub (or dev-only)                                              | —                                                               |

**Rule of thumb:** If the user would screenshot it for support or needs to **see system state**, it belongs in Brain Hub. If it’s **ephemeral intent** (“summarize this thread”), keep it in chat.

## Sections (evolutionary roadmap)

Order is **suggested priority**, not a commitment to ship all at once.

### 0. Wiki & System Health (Shipped)

- **Wiki Summary:** Doc count, last edit.
- **Recently Written:** Quick access to newest docs.
- **Background Agents:** Live activity and history (replaces bottom status bar).
- **Quick Links:** Profile (`me.md`), Email index status, **Sync Now** (replaces top-nav sync button).
...
- **Cross-link opportunities** instead of duplicating specs: Brain Hub **surfaces** tunnel and OAuth; OPP-008 / OPP-019 remain the **source of truth** for behavior.

## Relation to other opportunities

- **[OPP-008](./OPP-008-tunnel-qr-phone-access.md)** — QR and URL live in Brain Hub when implemented.
...
1. **Layout:** Full-page route vs slide-over vs hybrid (settings list as overlay, detail full-width)—currently `/hub` replaces main chat area.
2. **Auth:** Until multi-user productization, is Brain Hub always available in dev and gated in prod the same way as the rest of the app?
3. **Single source of truth for “custom instructions”:** Wiki page vs DB field vs file in `BRAIN_HOME`—Brain Hub should reflect the chosen architecture without a second copy.
...
2. Wire the first **real** section when **[OPP-008](./OPP-008-tunnel-qr-phone-access.md)** or **sources management** ships.
