...
Added `credentials: 'include'` to all inbox-related fetch calls in client components:

1. **`Inbox.svelte`**: All `/api/inbox/*` endpoints now include credentials:
   - `fetchInboxMessageForOpen` (line 268) — main GET message load
   - `load` — GET inbox list
   - `sync` — POST sync trigger
   - `archive` — POST archive
   - `markRead` — POST mark read
   - `loadContacts` — GET who (contacts)
   - Draft creation — POST reply/forward

2. **`Home.svelte`**: GET `/api/inbox` for home page preview

3. **`BrainHubPage.svelte`**: GET `/api/inbox/mail-sync-status` in hub data fetch

The browser now sends the `brain_session` cookie with all inbox requests, ensuring tenant and vault middleware can authenticate the user consistently with agent tool execution.

**Test coverage:** New test suite in [`src/server/routes/inbox.auth.test.ts`](../../src/server/routes/inbox.auth.test.ts) reproduces the 401 scenario and validates the fix.
