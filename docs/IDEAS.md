...
| [IDEA-brain-query-delegation](ideas/IDEA-brain-query-delegation.md) | Brain-query delegation (LLM-to-LLM fast path) | Delegate a research task to another tenant's LLM — "ask Donna what's latest on the construction project" — and get a synthesized answer filtered through a per-user privacy policy (LLM-instructed filter pass) before it leaves their instance. Higher-bandwidth than email, more powerful than file sharing; Phase 0 experiment possible on hosted instance with no new identity infrastructure. |
