...
| [IDEA-rl-rlhf-agent-quality-flywheel](ideas/IDEA-rl-rlhf-agent-quality-flywheel.md) | RL / RLHF agent quality flywheel | How preference-style optimization could apply to Braintunnel **later**; **early** focus on evals, **`/feedback`** as the structured human funnel, **draft→send** and iteration count as implicit signals, composer chips as weak positives—plus caveats (privacy, bias, volume). No OPP yet. |
