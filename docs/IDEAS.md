...
| [IDEA-wiki-sharing-collaborators](ideas/IDEA-wiki-sharing-collaborators.md) | Wiki sharing with collaborators (dirs, RW/RO) | Share vault subtrees (e.g. `trips/`) with another user — assistant travel collaboration — read-only or read-write; individuals before groups. **[OPP-064](opportunities/OPP-064-wiki-directory-sharing-read-only-collaborators.md)** is the first step (read-only invite, email-as-identity, **per-tenant share manifests**, symlink ergonomics — see [wiki-directory-sharing.md](architecture/wiki-directory-sharing.md)). Write access, agent-scope, handle-based identity, and conflict resolution remain open in the idea file. |
