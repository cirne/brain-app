...
| [OPP-010](opportunities/OPP-010-user-skills.md)                | User Skills (slash commands) | Slack-style `/` menu backed by `SKILL.md` files (Claude Code / Cursor / OpenClaw format) stored in `<WIKI_DIR>/skills/` — user-editable, git-versioned recipes like `/new`, `/tidy`, `/research`, `/prune`, `/draft`.     |
