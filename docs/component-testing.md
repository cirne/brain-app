...
End-to-end browser tests use **Playwright** against the Enron **demo tenant** in **`./data`** while **`pnpm run dev`** serves on **:3000** (Bearer mint): [`tests/e2e/`](../tests/e2e/), `pnpm run e2e:playwright`, [docs/architecture/enron-demo-tenant.md](architecture/enron-demo-tenant.md). Deterministic **ripmail** checks against the Kean tenant index: `pnpm run e2e:enron`.
