...
| BUG-015 | Onboarding profiling transcript: poor spacing / alignment | Profile-building view: **cramped** gap between lock + lead copy and the **pulse + status** line; **Search mail** tool hint **does not align** with the pulse column. Mixed **Tailwind `mb-4`** on the lead header vs **`.ob-prof-lead`** margins vs **`.ob-prof-activity`** `margin-top: 0` — check **margin collapse** and parent **`chat-transcript-scroll`**. See [bugs/BUG-015-onboarding-profiling-transcript-spacing.md](bugs/BUG-015-onboarding-profiling-transcript-spacing.md). |
...
| BUG-018 | Hub: Resume does not unpause Your Wiki | After **Pause** in Hub, **Resume** can leave the background agent **still paused**; supervisor may not restart. See [bugs/BUG-018-hub-resume-does-not-unpause-your-wiki.md](bugs/BUG-018-hub-resume-does-not-unpause-your-wiki.md). User feedback #1. |
