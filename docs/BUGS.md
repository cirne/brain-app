...
| BUG-012 | Agent tool path sandbox escape (cross-tenant / host FS) | **Security:** tools accept absolute paths and subprocess args without proving paths stay under the current `BRAIN_HOME` / tenant; prompts can exfiltrate other users’ trees on shared `BRAIN_DATA_ROOT` or any path the process can read. See [bugs/BUG-012-agent-tool-path-sandbox-escape.md](bugs/BUG-012-agent-tool-path-sandbox-escape.md). |
