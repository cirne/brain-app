...
| BUG-006 | Chat: concurrent POSTs same session → pi-agent error | One `Agent` per `sessionId`; overlapping `agent.prompt()` calls throw *Agent is already processing a prompt*. Server has no per-session queue; client double-send or two tabs can hit it. See [bugs/BUG-006-agent-concurrent-prompt.md](bugs/BUG-006-agent-concurrent-prompt.md). |
| BUG-007 | Agent view: stream deltas force scroll to bottom | While reading scrolled-up history, every token/tool update calls `scrollToBottom()` (`consumeAgentChatStream` in `agentStream.ts`), so the viewport jumps down. Need stick-to-bottom only when the user is already at (or near) the bottom. See [bugs/BUG-007-agent-view-scroll-stick-to-bottom.md](bugs/BUG-007-agent-view-scroll-stick-to-bottom.md). |
