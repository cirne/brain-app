...
| BUG-009 | Desktop: webview wrong URL / 404 when native port taken | Release Brain.app: if the default loopback port (e.g. `18473`) is in use by another session, the webview can load the wrong origin or show `404`. TCP-probe fix replaced with `BRAIN_LISTEN_PORT` stdout; **issue may persist** — see [bugs/BUG-009-desktop-webview-wrong-port.md](bugs/BUG-009-desktop-webview-wrong-port.md). |
...
| BUG-013 | Hosted: Gmail send via `ripmail` SMTP fails (DO blocks 587/465/25) | **Open:** Staging on DigitalOcean cannot open TCP to `smtp.gmail.com:587` (platform default); OAuth/HTTPS works. **Likely fix:** send via **Gmail API** on **443**, not third‑party SMTP. See [bugs/BUG-013-hosted-gmail-smtp-egress-blocked-digitalocean.md](bugs/BUG-013-hosted-gmail-smtp-egress-blocked-digitalocean.md). |
